...
logging.basicConfig(
    level=logging.INFO,
    format="%(message)s",  # Мы будем писать чистый JSON
)
logger = logging.getLogger("eda_api")


def log_event(endpoint: str, status: str, latency_ms: float, **kwargs: Any) -> None:
    """Пишет структурированный лог-событие в формате JSON."""
    event = {
...
def get_metrics() -> dict[str, Any]:
    ...
    return {
        ...
        "ok_for_model_count": METRICS["ok_for_model_count"],
        "ok_ratio": (
            round(METRICS["ok_for_model_count"] / METRICS["total_requests"], 2)
            if METRICS["total_requests"] > 0
            else 0.0
        ),
    }
...
# ---------- /quality-flags-from-csv: возвращаем набор флагов качества ----------


@app.post(
    "/quality-flags-from-csv",
    tags=["quality"],
    summary="Возвращает полный набор флагов качества CSV-файла"
)
...
# ---------- /head: возвращаем первые n строк ----------


@app.post(
    "/head",
    tags=["quality"],
    summary="Возвращает первые n строк CSV-файла"
)
async def head(
    ...
    """
    Принимает CSV-файл, возвращает первые n строк датасета в формате JSON.
    """
    start = perf_counter()
    METRICS["total_requests"] += 1
    ...
    return {"data": df.head(n).to_dict(orient="records")}
