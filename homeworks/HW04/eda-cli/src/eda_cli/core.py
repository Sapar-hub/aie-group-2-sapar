...
def compute_quality_flags(summary: DatasetSummary, missing_df: pd.DataFrame, zero_value_threshold: float = 0.15) -> Dict[str, Any]:
    ...
    # Т.к. unique - это количество уникальных значений то если это число равно единице
    # все значение равны в этой колонне. Вполне логично
    flags["has_constant_columns"] = any(c.unique == 1 for c in summary.columns)

    # Используем regular expressions для того чтобы найти все колонки с названием id - все идентификаторы
    # и если количество уникальных значений меньше чем сами значения то возвращаем True
