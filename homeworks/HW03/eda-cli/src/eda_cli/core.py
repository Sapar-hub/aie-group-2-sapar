...
def summarize_dataset(
    ...
    for name in df.columns:
        ...
        if is_numeric and non_null > 0:
            ...
            std_val = float(s.std())
            zero_frac = float(len(s[s==0])) / non_null # Доля нулевых значений
...
def compute_quality_flags(summary: DatasetSummary, missing_df: pd.DataFrame, zero_value_threshold: float = 0.15) -> Dict[str, Any]:
    """
    ...
    # Т.к. unique - это количество уникальных значений то если это число равно единице все значение равны в этой колонне. Вполне логично
    flags["has_constant_columns"] = any(c.unique == 1 for c in summary.columns)

    # Используем regular expressions для того чтобы найти все колонки с названием id - все идентификаторы и если
    # количество уникальных значений меньше чем сами значения то возвращаем True
    flags["has_suspicious_id_duplicates"] = any((re.findall(r"_id|-id|^id$",c.name, re.IGNORECASE) and c.unique < c.non_null) for c in summary.columns)

    flags["has_many_zero_values"] = any((c.is_numeric and c.zero_share is not None and c.zero_share > zero_value_threshold) for c in summary.columns)
