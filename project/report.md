...
**Финальный выбор: CV Baseline без position map (F1 = 0.727)**
...
3. **Hybrid v4 (F1 = 0.712)** всё ещё ниже чистого CV baseline (+0.034 к YOLO, но −0.015 к CV)
4. **Faster R-CNN v4 (F1 = 0.627)** занял последнее место — низкий recall (45.7%) из-за недостатка обучающих данных
5. **v3-модели (YOLO F1 = 0.857)** невоспроизводимы — метрики недостижимы в текущем окружении
6. **CV без обучения** — не требует GPU, данных, и даёт лучший F1 (0.727) среди воспроизводимых подходов
...
4. **Эксперименты:**
   ...
   - Гибрид v4: +0.034 F1 над YOLO v4, но CV baseline остаётся лучшей воспроизводимой моделью (F1 = 0.727)

5. **На что обратить внимание:**
   - **Честные метрики:** YOLO v4 F1 = 0.678 vs CV Baseline F1 = 0.727 — CV побеждает
