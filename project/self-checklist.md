...
|---|----------|--------|---------------------------|
| 1 | Сервис запускается по инструкциям из `README.md` и работает | ✅ | `src/api/main.py` — FastAPI приложение, запуск `python -m src.api.main`. **Важно:** модель ищется по пути `artifacts/yolo/exp01/weights/best.pt`. |
