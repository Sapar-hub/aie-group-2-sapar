...
|--------|---------------|---------|-----|------|--------|-----|------|
| CV Baseline | — | 35 real | 0.612 | 0.793 | 0.657 | 0.719 | 82.9% |
...
| Faster R-CNN | v2* | 45 real | 0.731 | 0.949 | 0.822 | **0.881** | 86.7% |

> **\*** Faster R-CNN v2 оценён на 45 изображениях (включает 10 val, не исключённых из теста) — не полностью сопоставим с остальными. Подробнее: [data/LEAKAGE.md](data/LEAKAGE.md).

> Hybrid v4 улучшает YOLO v4 на **+0.034 F1** (0.678 → 0.712) за счёт слияния CNN-уверенности с CV-признаками (aspect ratio, edge density). v3 не нуждается в уточнении — Precision=0.964 уже близка к насыщению за счёт "white canvas shortcut" в синтетических данных. Подробнее о данных: [data/LEAKAGE.md](data/LEAKAGE.md).
